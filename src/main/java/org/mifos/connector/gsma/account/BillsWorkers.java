...
public class BillsWorkers {
    ...
    public void setupWorkers() {
        ...
        zeebeClient.newWorker().jobType("billsPayment").handler((client, job) -> {
        ...
        }).name("billsPayment").maxJobsActive(workerMaxJobs).open();

        zeebeClient.newWorker().jobType("validateBill").handler((client, job) -> {
            logger.info("Job '{}' started from process '{}' with key {}", job.getType(), job.getBpmnProcessId(), job.getKey());
            Map<String, Object> variables = job.getVariablesAsMap();
            ...
            client.newCompleteCommand(job.getKey()).variables(variables).send().join();
        }).name("validateBill").maxJobsActive(workerMaxJobs).open();
    }
