    ...
    public void configure() throws Exception {
        ...
        from("direct:reversal-starter")
                ...
                .when(exchange -> exchange.getProperty(RESPONSE_OBJECT_AVAILABLE, Boolean.class))
                    .process(exchange -> exchange.setProperty(TRANSACTION_REFERENCE, exchange.getProperty(RESPONSE_OBJECT, GSMATransaction.class).getTransactionReference()))
                    .to("direct:reversal-base-route")
                .otherwise()
        ...
        from("direct:reversal-base-route")
                ...
                .log(LoggingLevel.INFO, "Got access token, moving on to get transaction reference")
                .process(exchange -> {
                ...
                .to("direct:commit-reversal")
                .log(LoggingLevel.INFO, "Reversal Response: ${body}")
                .choice()
                .when(header("CamelHttpResponseCode").isEqualTo("201"))
                    .log(LoggingLevel.INFO, "Reversal successful")
                .otherwise()
